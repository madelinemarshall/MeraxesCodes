...
def plot_magorrian(ax):
  ...
  cosmo['h']=0.678
  for z in zs:
    obs    = correlation(feature=feature,z_target=z,quiet=0,h=cosmo['h'])
...
def plot_MBHMstellar(filename,snapshots,mass_bulge,split,bulge_type,contours,color,axes):
  ...
  for snap in snapshots:
    ...
    sl,inter,rval,stderr_s,sterr_i=lsqfity(logMstel[logMBH>6],logMBH[logMBH>6])
    axes.plot([8.6,11.6],inter+sl*np.array([8.6,11.6]),'k',lw=2.5,label='$z={}$ (Tiamat-125-HR)')
    print("corr. coeff. = {}, mass_bulge = {}".format(lsqfity(logMstel[logMBH>6],logMBH[logMBH>6]),mass_bulge))
    ii+=1
...
if __name__=='__main__':
  ...
  lgd=ax[0].legend(fontsize='small',loc='upper center', bbox_to_anchor=(2.35, 0.825))  
