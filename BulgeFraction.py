...
def plot_disk_frac_SDSS(axes,bulge_frac):
  ...
  if bulge_frac:
    axes.plot(M,1-np.array(F),'--',label='Thanjavur et al. (2016)',color='k')
  else:
    axes.plot(M,F,'--',label='Thanjavur et al. (2016)',color='k')
...
if __name__=="__main__":
  ...
  for snapshot in [242]:
    ...
    if plot_mags:
      ...
      plot_disk_frac(gals_bulges[(mag_def<17.77)],axes_frac,r'$r<17.77$ - ',split,bulge_frac)
    else:
      plot_disk_frac(gals_bulges,axes_frac,'',split,bulge_frac)
  ...
  plt.savefig('/home/mmarshal/results/plots/BulgeFrac_z={}_points.pdf'.format(redshift[snapshot]),format='pdf',bbox_extra_artists=(lgd,), bbox_inches='tight')
  plt.show()
