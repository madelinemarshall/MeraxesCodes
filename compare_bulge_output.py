...
for ii in range(0,nfiles):
  ...
  gals["{}".format(filename[ii])] = gals["{}".format(filename[ii])][(gals["{}".format(filename[ii])]["StellarMass"]*1e10>1e9)]
...
for ii in range(0,nfiles):
  if (filename[ii]=='bulges_tiamat125')|(filename[ii]=='bulges_crotonSF_tiamat125'):
    vol=125/cosmo['h']
  else:
    vol=100
  hist, bin_edges = np.histogram(np.log10(gals["{}".format(filename[ii])]['BlackHoleMass']*1e10),range=(minval1,maxval1))
  ax1.plot(bin_edges[:-1],hist/(bin_edges[1]-bin_edges[0])/vol**3,label=filename[ii])
  ...
  ax2.plot(bin_edges[:-1],hist/(bin_edges[1]-bin_edges[0])/vol**3,label=filename[ii])
  ...
  ax3.plot(bin_edges[:-1],hist/(bin_edges[1]-bin_edges[0])/vol**3,label=filename[ii])
  ...
  ax4.plot(bin_edges[:-1],hist/(bin_edges[1]-bin_edges[0])/vol**3,label=filename[ii])
#  #Plot histograms of hot gas distribution
#  prop='HotGas'
#  hist, bin_edges = np.histogram(np.log10(gals["{}".format(filename[ii])][prop]*1e10),range=(minval3,maxval3))
...
#ax4.set_title('Hot Gas Mass Function')
#ax4.set_yscale('log')
