...
def plot_disk_frac(gals,axes,label,split,bulge_frac,mags):
  ...
  if split:
    if  not mags:
      ...
      axes.plot(BinStart[BinFull]+0.5*BinWidth,BFracInBin[BinFull][:,1],label=label+r'Total Bulge',lw=2,color=color[2],linestyle=(0, (1, 1)))
      ...
      axes.plot(BinStart[BinFull]+0.5*BinWidth,BFracInBin[BinFull][:,1],'--',label=label+r'Total Bulge',lw=3,color=color[2],zorder=100,markersize=3)
  ...
  axes.set_ylabel(r'$M_{\textrm{bulge}}/M_\ast$')
  #print(err)
  #print(1-FracInBin)
...
def plot_disk_frac_SDSS(axes,bulge_frac):
  ...
  if bulge_frac:
    ...
    #var.set_dashes([2,2,5,2])
    axes.plot(M,1-np.array(F),'--',label='Thanjavur et al. (2016)',color=[0.4,0.4,0.4],markersize=3,lw=2)
  else:
...
def plot_bulge_frac_GAMMA(axes,bulge_frac):
  ...
  if bulge_frac:
    #axes.errorbar(M,F,yerr=[F-F_low,F_up-F],marker='o',label='Moffett et al. (2016)',color=[0.5,0.5,0.5],linestyle='--',markersize=3)
...
if __name__=="__main__":
  ...
  for snapshot in [242]:
    ...
    fig_frac,axes_frac=plt.subplots(1,3,gridspec_kw={'width_ratios':[4,4,2]})
    plot_disk_frac(gals_bulges,axes_frac[0],'',0,bulge_frac,0)
    plot_disk_frac(gals_bulges,axes_frac[1],'',1,bulge_frac,0)
    if plot_mags:
      mag_def=load_mags(filename,snapshot)#['i775']
      mag_def=mag_def[selection]
      plot_disk_frac(gals_bulges[(mag_def<17.77)],axes_frac[0],r'$r<17.77$ - ',split,bulge_frac,1)
      plot_disk_frac(gals_bulges[(mag_def<19.8)],axes_frac[0],r'$r<19.8$ - ',split,bulge_frac,1)
  
  plot_disk_frac_SDSS(axes_frac[0],bulge_frac)
  plot_bulge_frac_GAMMA(axes_frac[0],bulge_frac)
  plot_bulge_frac_EAGLE(axes_frac[0],bulge_frac)
  axes_frac[0].plot([8.65,8.65],[-3,2],'--',color=[0.5,0.5,0.5],label='Convergence Limit')
  axes_frac[1].plot([8.65,8.65],[-3,2],'--',color=[0.5,0.5,0.5],label='Convergence Limit')
  plt.tight_layout()
  axes_frac[2].axis('off')
  lgd=axes_frac[0].legend(fontsize='small',loc='upper center', bbox_to_anchor=(2.77, 0.8))
  lgd2=axes_frac[1].legend(fontsize='small',loc='upper center', bbox_to_anchor=(1.45, 0.4))
