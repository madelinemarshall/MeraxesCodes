...
def plot_all_props(redshift,BHproperties_bulge):
...
if __name__=='__main__':
  ...
  BHproperties_orig=np.zeros((13,len(ID),len(redshift)))
  ...
  addcolumns_orig=['ID','Type','CentralGal','ColdGas','HotGas','StellarMass','DiskScaleLength','Sfr','Rvir','Mvir','Vvir']
  columns_orig=['BHmass','ID','Type','CentralGal','ColdGas','HotGas','StellarMass','DiskScaleLength','Sfr','Rvir','Mvir','Vvir','dMvir','dClass','XPos','YPos','ZPos']
  ...
  for galID in ID:
    ...
    for name in addcolumns:
      ...
      i=i+1
    fname='/home/mmarshal/PhD/results/history/'+str(int(galID))+'/BlackHoleMass.bin'
    BHmass=np.fromfile(fname,dtype=float,count=-1,sep="")
    BHproperties_orig[0,idx,:]=BHmass
    i=1
    for name in addcolumns_orig:
      fname='/home/mmarshal/PhD/results/history/'+str(int(galID))+'/'+name+'.bin'
      col=np.fromfile(fname,dtype=float,count=-1,sep="")
      BHproperties_orig[i,idx,:]=col
      i+=1
    idx=idx+1
  ...
  for i in range(0,len(ID)):
    f, (ax1, ax2, ax3) = plt.subplots(1,3,figsize=(16,3))
    ax1.plot(redshift[30:],np.log10(BHproperties_bulge[7,i,30:]*1e3))
    ax1.plot(redshift[30:],np.log10(BHproperties_orig[7,i,30:]*1e3))
    ax1.set_xlabel('Redshift')
    ax1.invert_xaxis()
    ax1.set_ylabel('Radius')
    
    ax2.plot(redshift[30:],np.log10(BHproperties_bulge[0,i,30:]*1e10))
    ax2.plot(redshift[30:],np.log10(BHproperties_orig[0,i,30:]*1e10))
    ax2.set_xlabel('Redshift')
    ax2.invert_xaxis()
    ax2.set_ylabel('Black Hole Mass')
    
    ax3.plot(redshift[30:],np.log10(BHproperties_bulge[4,i,30:]/BHproperties_bulge[-1,i,30:]))
    ax3.set_xlabel('Redshift')
    ax3.invert_xaxis()
    ax3.set_ylabel('Black Hole / Bulge Mass')
