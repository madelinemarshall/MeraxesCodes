...
def load_data(filename,snapshot):
  ...
  gals=gals[gals['BlackHoleMass']*1e10>0]
...
def find_med(gals):
  med=np.log10(np.median(gals['StellarMass']*1e10))
  #eightyfourth_pctile=np.log10(np.percentile(gals['StellarMass']*1e10,84))
  #sixteenth_pctile=np.log10(np.percentile(gals['StellarMass']*1e10,16))
  return med #[med,eightyfourth_pctile,sixteenth_pctile]
  

def stu_approx(Mh,z,Mstellar0):
  #return Mstellar0*(2*(Mh/1e12)**(2/3)*(1+z)+1)/(6*(Mh/1e12)**(2/3)+1)
  #top=(1+z)**2*(0.5*(Mh/1e12)**(2/3)*3+35)
  #bottom=(0.5*(Mh/1e12)**(2/3)*(1+z)+35)*9
  top=(1+z)*(0.5+70**2/(141*(Mh/1e12)**(2/3)*(3)))
  bottom=(0.5+70**2/(141*(Mh/1e12)**(2/3)*(1+z)))*3
  return Mstellar0*top/bottom
...
if __name__=="__main__":
  ...
  color={9:'C0',9.5:'C1',10:'C2',10.5:'C3',11:'C4',11.5:'pink',12:'black'}
  ...
  for up in [9.5,10,10.5,11,11.5,12]:
    med={}
    #pctile84={}
    #pctile16={}
    med_def={}
    ...
    for snapshot in snapshots:
      ...
      med[snapshot]=find_med(gals[(gals['Mvir']*1e10>10**(up-0.1))&(gals['Mvir']*1e10<10**up)])
      med_def[snapshot]=find_med(gals_def[(gals_def['Mvir']*1e10>10**(up-0.1))&(gals_def['Mvir']*1e10<10**up)])
    ...
    #axes.plot(np.array(list(redshift.values())),np.log10(stu_approx(10**up,np.array(list(redshift.values()),float),10**med[158])),'k:',label='__nolegend__')
