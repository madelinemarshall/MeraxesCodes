...
def load_data(filename,snapshot,split):
  ...
  if split:
    gals=meraxes.io.read_gals(data_folder+filename+meraxes_loc,\
      snapshot=snapshot,props=['GhostFlag','Mvir','StellarMass','BlackHoleMass','CentralGal','BulgeStellarMass','MergerBulgeStellarMass','BlackHoleMass_ID','BlackHoleMass_MD'],\
...
def plot_MBHMstellar(filename,snapshots,mass_bulge,split,bulge_type,contours,color,axes):
  ...
  for snap in snapshots:
    ...
    if mass_bulge==0:
      ...
      if split:
        ...
        if bulge_type==2:
          ...
          MBH=gals['BlackHoleMass_MD']*1e10
        #ID bulge mass
        if bulge_type==1:
          ...
          MBH=gals['BlackHoleMass_ID']*1e10
      else:
        gals=gals[gals['BulgeStellarMass']>0]
        Mbulge=gals['BulgeStellarMass']*1e10
        MBH=gals['BlackHoleMass']*1e10
    ...
    if (snap==158)&contours:
      print('Plotting Contour')
      cp.contour_plot(logMstel,logMBH,xlab=None,ylab=None,xlims=[8.7,11.6],ylims=[5,9.5],axes=axes,colors=color[snap],levels=np.logspace(-2,1,7),linewidth=0.9)
    ii+=1
  axes.set_xlim([8.7,11.6])
  axes.set_ylim([5,9])
...
if __name__=='__main__':
  ...
  redshift={52:8,63:7,78:6,100:5,116:4,134:3,158:2,250:0}
  snapshots=[63,78,100,116,134,158]
  #snapshots=[116,134,158]
  prop='StellarMass'
  color={52:'C0',63:'C1',78:'C2',100:'C3',116:'C4',134:'aqua',158:'pink',250:'k'}
  color={52:'#a65628',63:'#e41a1c',78:'#377eb8',100:'#4daf4a',116:'#984ea3',\
                  134:'#ff7f00',158:'#f781bf',194:'#a65628',250:'black'}
  ...
  if plot_type==1: 
    ...
    ax[0].set_ylabel(r'$\log(\textrm{M}_{\textrm{instability-driven BH}})$') 
    ax[1].set_ylabel(r'$\log(\textrm{M}_{\textrm{merger-driven BH}})$') 
    ax[1].yaxis.tick_right()
    ax[1].yaxis.set_label_position("right")

  plt.legend()
  lgd=plt.legend(fontsize='small',loc='upper center', bbox_to_anchor=(1.31, 0.82))  
  plt.tight_layout()
  if plot_type==1:
