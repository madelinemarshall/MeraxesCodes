...
def plot_obsBHMF_z0pt0(ax,hubble_h=0.678,markersize=8,legend=True,color='gray'):
    ...
    if legend:
        ax.errorbar(data[:,0],data[:,1]*1e-4*_h_convertor_y(0.7,hubble_h),\
                     ...
                     markersize=markersize,color=color,fmt='o',label='Graham et al. 2007')
    else:
        ax.errorbar(data[:,0],data[:,1]*1e-4*_h_convertor_y(0.7,hubble_h),\
                     yerr=[data[:,3]*1e-4*_h_convertor_y(0.7,hubble_h),data[:,2]*1e-4*_h_convertor_y(0.7,hubble_h)],\
                     markersize=markersize,color=color,fmt='o')
    ...
    if legend:
        ax.errorbar(data[:,0],data[:,1]*1e-4*_h_convertor_y(0.7,hubble_h),\
                     yerr=[data[:,3]*1e-4*_h_convertor_y(0.7,hubble_h),data[:,2]*1e-4*_h_convertor_y(0.7,hubble_h)],\
                     markersize=markersize,color=color,fmt='o',mfc='None',label='Vika et al. 2009')
    else:
        ax.errorbar(data[:,0],data[:,1]*1e-4*_h_convertor_y(0.7,hubble_h),\
                     yerr=[data[:,3]*1e-4*_h_convertor_y(0.7,hubble_h),data[:,2]*1e-4*_h_convertor_y(0.7,hubble_h)],\
                     markersize=markersize,color=color,fmt='o',mfc='None')
    ...
    if legend:
        ax.errorbar(data[:,0],data[:,1]*1e-4*_h_convertor_y(0.6777,hubble_h),\
                     ...
                     markersize=markersize,color=color,fmt='v',label='Davis et al. 2014')
    else:
        ax.errorbar(data[:,0],data[:,1]*1e-4*_h_convertor_y(0.6777,hubble_h),\
                     yerr=[data[:,3]*1e-4*_h_convertor_y(0.6777,hubble_h),data[:,2]*1e-4*_h_convertor_y(0.6777,hubble_h)],\
                     markersize=markersize,color=color,fmt='v')
    ...
    if legend:
        ax.errorbar(data[:,0],10**data[:,4]*_h_convertor_y(0.6777,hubble_h),\
                     ...
                     color=color,fmt='s',markersize=markersize,label='Mutlu-Pakdil et al. 2016')
    else:
        ax.errorbar(data[:,0],10**data[:,4]*_h_convertor_y(0.6777,hubble_h),\
                     yerr=[(10**(data[:,4]+data[:,5])-10**data[:,4])*_h_convertor_y(0.6777,hubble_h),\
                           (10**data[:,4]-10**(data[:,4]-data[:,6]))*_h_convertor_y(0.6777,hubble_h)],\
                     color=color,fmt='s',markersize=markersize)
...
def plot_obsBHMF_z0pt5(ax,hubble_h=0.678,markersize=10,legend=True,color='gray'):
    ...
    ax.fill_between(data[:,0],10**data[:,1]*_h_convertor_y(0.7,hubble_h),10**data[:,5]*_h_convertor_y(0.7,hubble_h),lw=0,color=color,alpha=0.5)#,hatch='\\\\\\',edgecolor='k')

def plot_obsBHMF_legend(ax,markersize=10,color='gray',alpha=1.0):
    hubble_h=0.678
...
def plot_obsBHMF(ax,z,hubble_h=0.678,markersize=10,legend=True,silent=False,color='gray',alpha=1.0):
    _plot_obsBHMF = {0.5: plot_obsBHMF_z0pt5,\
                     0.0: plot_obsBHMF_z0pt0}
    try:
        (_plot_obsBHMF[z])(ax,hubble_h=hubble_h,markersize=markersize,legend=legend,color=color)
    except:
        if not silent:
