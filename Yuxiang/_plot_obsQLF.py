...
def fitting_jiang16(M):
    return 9.93e-9/(10**(0.4*(-25.2-M)*(1.9-1))+10**(0.4*(-25.2-M)*(2.8-1)))
...
def plot_obsQLF_z6pt0(ax,markersize=10,legend=True,hubble_h=0.678):
    if legend:
        ...
        x = np.linspace(-22,-28)
        ax.plot(x,fitting_jiang16(x),color='gray',linestyle='--',lw=3, label='Jiang et al. 2016')
    else:
        ...
        x = np.linspace(-22,-28)
        ax.plot(x,fitting_jiang16(x),color='gray',linestyle='--',lw=3)
...
def plot_obsQLF_legend(ax,markersize=10,color='gray',alpha=1.0):
...
def plot_obsQLF(ax,z,markersize=10,legend=True,hubble_h=0.678,silent=False,color='gray',alpha=1.0):
