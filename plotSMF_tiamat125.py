...
def plot_SMF(gals,prop,vol,axes,label):
    ...
    axes.plot(Max,np.log10(hist/(bin_edges[1]-bin_edges[0])/vol**3),linewidth=3.0,label=label)
    return axes
...
#                     0.55:
  ...
  ii=-1
  j=0
  for snapshot in np.flip([213,158,134,116,78,63],0):#[63,78,100,116,134,158]:
    ii+=1
    if ii==3:
      j+=1
      ii=0
    ...
    axes[j,ii].text(10.1, -1.9, r'$z={}$'.format(redshift[snapshot]),weight='bold',size='x-large')
    axes[j,ii].set_xlim([7.5,12.5])
    axes[j,ii].set_ylim([-7.3,-1.3])
