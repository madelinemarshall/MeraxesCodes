...
def plot_Davis(axes):
    ...
    axes.plot(davis[0],davis[1],'-.',color=[0.3,0.3,0.3],label=r"Davis et al. (2014) $z=0$",markersize=3,lw=1.5)
    axes.fill_between(davis[0],davis[2],davis[3],color=[0.3,0.3,0.3],alpha=0.4)


def plot_Shankar(axes):
    shan=pd.read_csv('/home/mmarshal/simulation_codes/data/Shankar_BHMF.csv',header=None)
    axes.plot(shan[0],shan[1],':',color=[0.5,0.5,0.5],label=r"Shankar et al. (2009) $z=0$",markersize=3,lw=1.5)
    axes.fill_between(shan[0],shan[3],shan[2],color=[0.5,0.5,0.5],alpha=0.4)


def plot_derived(axes):
    MBH=np.load('data/BestFitBHMF_M.npy')
    log_phi=np.load('data/BestFitBHMF_phi.npy')
    axes.plot(MBH,log_phi,'k--')
...
if __name__=="__main__":
  ...
  boxwidth=100
  filename125='paper2_evolvingEdd_T125'
  plot_z0=1
  ...
  axes.set_xlim([5.8,10])
  axes.set_ylim([-4.8,-1.6])
  ...
  plt.savefig('/home/mmarshal/results/plots/Paper2/BHMF_T125_EddTest.pdf',format='pdf',bbox_extra_artists=(lgd,), bbox_inches='tight')
  plt.show()
